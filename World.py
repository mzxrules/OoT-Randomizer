...
class World(object):
    ...
    def __init__(self, settings):
        ...
        # TODO: Big poes aren't a thing in MM, right?
        # evaluate settings (important for logic, nice for spoiler)
        if self.big_poe_count_random:
            self.big_poe_count = random.randint(1, 10)
        # TODO: I'm guessing this means "time of day"?
        # Time is handled very differently in MM, so this will have to be
        # adjusted or just removed, probably.
        ...
        # Not sure what these were for, but will keep it
        # in for consistency.
        #
        # group a few others
        self.tunic_colors = [
            self.hatcolor, self.shirtcolor, self.pantscolor,
            self.swordcolor, self.deityswordcolor,
            self.dekuspincolor, self.zoraslashcolor,
            self.deitybeamcolor, self.boomerangtrail
            ]
        self.tunic_palettes = [
            self.dekucolor, self.dekutuniccolor,
    ...
    # This is where the JSON files get read and the
    # Regions, Locations and Entrances are parsed.
    ...
    def find_items(self, item):
        return [location for location in self.get_locations()
    ...
    def push_item(self, location, item):
        ...
        if location.can_fill_fast(item):
            ...
            logging.getLogger('').debug('Placed %s [World %d] at %s [World %d]',
                                        item,
                                        item.world.id if hasattr(item, 'world') else -1,
                                        location,
    ...
    def has_beaten_game(self, state):
        return state.has("Majora's Mask")
